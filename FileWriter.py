...
def csvWriter(processedDf, outputPath, mode ='overwrite', *paritionColumns):
    if requiredPartitions(paritionColumns):
        processedDf.write.mode(mode).partitionBy(*paritionColumns).csv(outputPath)
    else:
...
def jsonWriter(processedDf, outputPath, mode ='overwrite', *paritionColumns):
    if requiredPartitions(paritionColumns):
        processedDf.write.mode(mode).partitionBy(*paritionColumns).json(outputPath)
    else:
...
def parquetWriter(processedDf, outputPath, mode ='overwrite', *paritionColumns):
    if requiredPartitions(paritionColumns):
        processedDf.write.mode(mode).partitionBy(*paritionColumns).parquet(outputPath)
    else:
...
def text(processedDf, outputPath, mode ='overwrite', *paritionColumns):
    if requiredPartitions(paritionColumns):
        processedDf.write.mode(mode).partitionBy(*paritionColumns).text(outputPath)
    else:
...
def xmlWriter(processedDf, outputPath, mode ='overwrite', *paritionColumns):
    if requiredPartitions(paritionColumns):
        processedDf.write.mode(mode).partitionBy(*paritionColumns).format('xml').save(outputPath)
    else:
